...
public class Snake2 extends Actor
{
    ...
    public void act()
    {
        if(alive)
        {
            if(ai)
            {
                /*if(getOneObjectAtOffset((int)Math.cos(direction*90), (int)Math.sin(direction*90), Snake1.class)!=null)
                {
            ...
            nextNode = new Snake1(length-1, direction*90, nextNode);
            getWorld().addObject(nextNode, getX(), getY());
            move(1);
            if(getOneIntersectingObject(Food.class)!=null)foundFood();
            if(getOneIntersectingObject(Snake1.class)!=null||isAtEdge())death();
        }
        else
        {
    ...
    public Snake2(boolean aiControlled)
    {
        if(ai)ai = true;
        else ai = false;
        alive = true;
